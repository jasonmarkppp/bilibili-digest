...
async function startDigest(videoId, videoUrl) {
  ...
  if (generation !== digestGeneration || videoId !== currentVideoId) return;
...
function buildMarkdownExport() {
  ...
  lines.push("", "---", "由 Bilibili Digest 导出");
  return lines.join("\n");
}
...
function buildHtmlExport() {
  ...
  </style></head><body><main class="page"><h1>${escapeHtml(currentVideoTitle || "B站视频学习笔记")}</h1><div class="meta">UP主：${escapeHtml(currentChannelName || "未知")} · <a href="${escapeHtml(currentCanonicalVideoUrl())}">打开原视频</a></div>${currentVideoDescription ? `<h2>视频简介</h2><p>${escapeHtml(currentVideoDescription)}</p>` : ""}${chapters ? `<h2>AI 章节</h2>${chapters}` : ""}${quotes ? `<h2>关键观点</h2>${quotes}` : ""}<h2>完整字幕</h2>${entries.map((entry) => `<div class="line"><a class="time" href="${escapeHtml(entry.url)}">${escapeHtml(entry.timestamp)}</a><div>${escapeHtml(entry.text)}</div></div>`).join("")}<div class="footer">由 Bilibili Digest 导出</div></main></body></html>`;
}
...
function exportTranscript() {
  ...
  const baseName = sanitizeFilename(currentVideoTitle) || "bilibili-digest";
  if (format === "html") {
...
function showConfigError(configStatus) {
  ...
  document.getElementById("errorMessage").textContent =
    `请先在 Bilibili Digest 设置中填写${missingKeys.join("和")}。`;
...
async function loadFromCache(videoId) {
  ...
  try {
    ...
    if (
      cached.schemaVersion !== DIGEST_CACHE_SCHEMA_VERSION ||
